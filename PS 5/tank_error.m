...
for t = 1:T
    ...
    %error(T+t,1) = x(2*T+t)-x(2*T+t-1)-(1/params.sigma)*(params.phipi*x(T+t-1)+z(T+t-1)-x(T+t));
    ...
    %error(2*T+t,1) = x(t)-x(t-1)-(1/params.sigma)*(params.phipi*x(T+t-1)+z(T+t-1)-x(T+t));
    error(2*T+t,1) = x(t+1)-x(t)-(1/params.sigma)*(params.phipi*x(T+t)+z(T+t)-x(T+t+1));
    % pi(t)-beta*pi(t+1)-kappa*(y(t)-a(t)) equation error
    %error(3*T+t,1) = x(T+t-1)-params.beta*x(T+t)-kappa+(x(t-1)-z(t-1));
